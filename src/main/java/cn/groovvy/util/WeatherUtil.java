...
//        return parseObj(parseObj(response.body()).getJSONArray("location").get(0)).getStr("id");
    ...
    public static String getNowWeather(String city) {
        ...
        try {
            response = RetryUtil.getRetryTemplate().execute((RetryCallback<HttpResponse, Throwable>) retryContext -> HttpUtil.createGet(String.format(NOW_WEATHER_API, locationId))
    ...
    public static Weather getWeatherInfo(String city) {
        ...
        HttpResponse response = null;
        try {
            response = RetryUtil.getRetryTemplate().execute((RetryCallback<HttpResponse, Throwable>) retryContext -> HttpUtil.createGet(String.format(WEATHER_API, locationId))
                    .setConnectionTimeout(30000)
                    .setReadTimeout(40000)
                    .execute());
        } catch (Throwable throwable) {
    ...
    public static Map<String, Indices> getIndices(String city) {
        ...
        HttpResponse response = null;
        try {
            response = RetryUtil.getRetryTemplate().execute((RetryCallback<HttpResponse, Throwable>) retryContext -> HttpUtil.createGet(String.format(INDICES_API, locationId))
                    .setConnectionTimeout(30000)
                    .setReadTimeout(40000)
                    .execute());
        } catch (Throwable throwable) {
